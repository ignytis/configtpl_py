...
class TestJinjaGlobals(unittest.TestCase):
  ...
  def test_jinja_global_cmd(self, mock_run: MagicMock) -> None:
    ...
    mock_run.assert_called_once_with("echo Hello", shell=True, check=True, capture_output=True, text=True)  # noqa: S604

  @patch("os.getenv")
  def test_jinja_global_env_with_value(self, mock_getenv: MagicMock) -> None:
  ...
  @patch("os.getenv")
  def test_jinja_global_env_with_default(self, mock_getenv: MagicMock) -> None:
    env_vars = {"TEST_VAR": "mocked_value"}
    mock_getenv.side_effect = lambda key, default=None: env_vars.get(key, default)

    assert jinja_global_env("TEST_VAR", "default_value") == "mocked_value"
    assert jinja_global_env("THIS_DOES_NOT_EXIST", "default_value") == "default_value"
